...
interface Props { documents: Document[]; selectedId: string | null; activeIds: string[]; onSelect: (document: Document) => void; onToggleActive: (document: Document) => void; onUploaded: (document: Document) => void; onDeleted: (documentId: string) => void; loading: boolean; open: boolean; collapsed: boolean; onToggleCollapse: () => void; userEmail: string | null; onSignOut: () => void; }
...
export default function Sidebar({ documents, selectedId, activeIds, onSelect, onToggleActive, onUploaded, onDeleted, loading, open, collapsed, onToggleCollapse, userEmail, onSignOut }: Props) {
  ...
  return <aside className={`sidebar ${open ? "is-open" : ""} ${collapsed ? "is-collapsed" : ""}`} aria-label="Document library">
    <div className="brand"><span className="brand-mark" aria-hidden="true"/><span className="brand-name">Lumen</span><span className="brand-tag">READER</span><button className="sidebar-collapse" onClick={onToggleCollapse} aria-label={collapsed ? "Expand document library" : "Collapse document library"} title={collapsed ? "Expand library" : "Collapse library"}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d={collapsed ? "M9 18l6-6-6-6" : "M15 18l-6-6 6-6"}/></svg></button></div>
    ...
    <div className="library">
      ...
      {!loading && documents.length === 0 && <p className="empty-library">Your first source will live here. Add a PDF, CSV, or XLSX to begin.</p>}
        ...
        return <li key={document.id}>
          {isPendingDelete ? (
            ...
            <div className={`document-row ${selectedId === document.id ? "is-selected" : ""}`}>
              ...
              {document.status === "ready" && <button className={`document-session ${activeIds.includes(document.id) ? "is-active" : ""}`} aria-label={`${activeIds.includes(document.id) ? "Remove" : "Add"} ${document.filename} ${activeIds.includes(document.id) ? "from" : "to"} active sources`} title={activeIds.includes(document.id) ? "Remove from active sources" : "Add to active sources"} onClick={() => onToggleActive(document)}>{activeIds.includes(document.id) ? "−" : "+"}</button>}
