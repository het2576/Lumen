...
export default function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  const status = message.verification === "verified" ? "Verified data" : message.verification === "inferred" ? "Text only · verify" : "Sources checked";
  return <article className={`message ${isUser ? "user" : ""} ${message.error ? "error" : ""}`}><span className="message-avatar">{isUser ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><circle cx="12" cy="8" r="3.5"/><path d="M4.5 21a7.5 7.5 0 0 1 15 0"/></svg> : <LumenGlyph/>}</span><div className="message-body"><div className="message-byline"><strong>{isUser ? "You" : "Lumen"}</strong>{!isUser && !message.pending && <span>{message.verification === "verified" ? "verified answer" : "grounded answer"}</span>}</div><div className="message-content">{message.pending ? <span className="typing" aria-label="Lumen is thinking"><i/><i/><i/></span> : isUser || message.error ? message.content : <AnswerContent content={message.content}/>}</div>{message.chart && <LumenChart type={message.chart.type} title={message.chart.title} subtitle={message.chart.subtitle} data={message.chart.data}/>} {!isUser && !message.pending && !message.error && <div className="response-actions"><button onClick={copyAnswer} className="response-action" aria-label="Copy answer"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><rect x="9" y="9" width="11" height="11" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>{copied ? "Copied" : "Copy answer"}</button><span className="response-divider"/><span className={`response-status ${message.verification === "inferred" ? "is-caution" : ""}`}><i/>{status}</span></div>}{!isUser && message.sources && <SourceList sources={message.sources}/>}</div></article>;
}
